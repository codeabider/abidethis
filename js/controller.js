...
shopApp.controller("homeController",function($scope, $rootScope, productData, itemService){
  ...
  if(JSON.parse(sessionStorage.getItem("cartItems"))){
      for(j=0;j<JSON.parse(sessionStorage.getItem("cartItems")).length;j++){
        var the_string = "cartString"+JSON.parse(sessionStorage.getItem("cartItems"))[j]; /*showing items in cart after page refresh*/
  ...
  $scope.addToCart=function(){                            /*function called onClick of cartIcon*/                                               
    ...
    if(JSON.parse(sessionStorage.getItem("cartItems"))){
      for(j=0;j<JSON.parse(sessionStorage.getItem("cartItems")).length;j++){              /* retaining all the products from existing cart from session storage*/
        var the_string = "cartString"+JSON.parse(sessionStorage.getItem("cartItems"))[j];
        $scope[the_string]="true";
      }
    } 
  }
  ...
  if(JSON.parse(sessionStorage.getItem("cartItems"))){
    $rootScope.cartcount=JSON.parse(sessionStorage.getItem("cartItems")).length; /*retaining cart count after refresh*/
  }
  else{
  ...
  if(JSON.parse(sessionStorage.getItem("starItems"))){
    for(j=0;j<JSON.parse(sessionStorage.getItem("starItems")).length;j++){
      var star_string = "starString"+JSON.parse(sessionStorage.getItem("starItems"))[j];  /*showing items in wishlist after refresh*/
  ...
  $scope.addToFav=function(event){                                                                   
    ...
    if(JSON.parse(sessionStorage.getItem("starItems"))){     
      for(j=0;j<JSON.parse(sessionStorage.getItem("starItems")).length;j++){     /*getting products from session on click into initial array*/
        prod_id[j]=JSON.parse(sessionStorage.getItem("starItems"))[j];
      }
    }
    ...
    if(JSON.parse(sessionStorage.getItem("starItems"))){
      for(j=0;j<JSON.parse(sessionStorage.getItem("starItems")).length;j++){     /*retaining all the products from existing wishlist from session storage*/
        var star_string = "starString"+JSON.parse(sessionStorage.getItem("starItems"))[j];
        $scope[star_string]="true";
      }
    }
    
    for(i=1;i<JSON.parse(sessionStorage.getItem("starItems")).length;i++){
      if(JSON.parse(sessionStorage.getItem("starItems"))[i-1]==event.currentTarget.id){ 
        prod_id.pop(i);
  ...
  $rootScope.favcount=JSON.parse(sessionStorage.getItem("starItems")).length; /*display wishlist count on click*/   
}

if(JSON.parse(sessionStorage.getItem("starItems"))){
  $rootScope.favcount=JSON.parse(sessionStorage.getItem("starItems")).length;    /*retaining cart count after refresh*/
}
else{
...
shopApp.controller("productController",function($scope,$rootScope,productData,itemService){
  ...
  if(JSON.parse(sessionStorage.getItem("cartItems"))){
    for(j=0;j<JSON.parse(sessionStorage.getItem("cartItems")).length;j++){
      var the_string = "cartString"+JSON.parse(sessionStorage.getItem("cartItems"))[j]; /*showing items in cart after refresh*/
      $scope[the_string]="true";
    }
  }
...
if(JSON.parse(sessionStorage.getItem("starItems"))){
  ...
  $rootScope.favcount=0;
     ...
     /*logic for storing alphabetically goes here*/
